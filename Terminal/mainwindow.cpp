    ...
    ui(new Ui::MainWindow)
{
   ...
   // qDebug()<<"okoko";
    ...
    //Tab Map
    ui->webView->setPage( new QWebPage(this) );
    loadHtmlPage();
...
void MainWindow::updateListSensor()
{
    ...
    ui->webView->page()->mainFrame()->evaluateJavaScript("removeDirArrows()");
    FileData file(DATA_PATH);
    FileData data(HISTORY_FILE);
    int N = file.length();
    int M = data.length();
...
int MainWindow::getIndexMarker(int mac)
{
    ...
    for(int i = 0; i < N; i++)
    {
        if(ListSensor[i]->mac == mac)
            return ListSensor[i]->markerIndex;
    }
    return -1;
}
...
//    FileData file(DATA_PATH);
    ...
    //tmp = "\nUAV nhan duoc nhiet do tu node";
    tmp = "\nThong tin nhiet do tu sensor ";
...
{
    QString dataSend, url2, url1;
    if((!DATA::lat.isEmpty())&&(!DATA::lng.isEmpty())) //du Lieu GPS
    {
        dataSend = "UB:000" + DATA::lat + DATA::lng;
        url2 = "http://lab411s.esy.es/sg/ubuReceivePos.php?data=";
        url2 += dataSend;
    }
    if((!DATA::temp.isEmpty())&&(!DATA::hump.isEmpty()))
    {
        dataSend = "H" + DATA::mac + DATA::Ip + DATA::temp + DATA::hump;
        url1 = "http://lab411s.esy.es/sg/rx.php?data=";
        url1 += dataSend;
    }
    QNetworkRequest request2 = QNetworkRequest(QUrl(url2));
    QNetworkRequest request1 = QNetworkRequest(QUrl(url1));
    http1->get(request2);
    http1->get(request1);
}
...
//            dis[i] = distance(21.005565,105.842328, Nodelat, Nodelng);
    ...
    if(index==0){
        if(ListSensorStt[0])
            console->insertPlainText("Huong bay UAV: 0.836, 0.547 ");
        else
...
{
    QString cmd = "FFFF400$";
    //console->insertPlainText("UAV gui lenh thu thap nhiet do\n");
    tranceiver->writeData(cmd);
